...
export class TripCreatorService {
  ...
  getNewActivities(tripStep: TripStep, interests: { name: string, code: string }[]): Promise<TripStep> {
    return lastValueFrom(this.httpClient.post(environment.server_url + "steps/getNewStepActivities", {
      "step": tripStep,
      "interests": interests
    }))
      .then((response: any) => {
        if (response) {
          let step = TripStep._mapJsonToTripStep(response);
